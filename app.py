...
출력 JSON 포맷:
{{
  "summary": "메일의 핵심 내용을 2-3문장으로 상세히 요약. 발신자, 주요 내용, 요청사항을 포함하되 정중한 경어체 사용",
...
def get_fallback_ai_analysis(title):
    ...
    if "매출" in title_lower or "실적" in title_lower or "보고" in title_lower:
        return "상반기 매출 실적 보고서 작성 관련 안내입니다. 각 팀별 매출 데이터를 취합하여 특정 기한까지 제출해야 하며, 특히 신규 프로젝트 성과 부분을 상세히 기술할 필요가 있습니다. 누락 시 전체 마감에 지장이 있으므로 기한 준수가 중요합니다.", "데이터 취합 후 기한 내 회신", "High", "업무"
    elif "항공권" in title_lower or "결제" in title_lower or "예약" in title_lower:
        return "항공권 결제 및 예약 완료 안내입니다. 탑승 전 공항 카운터에서 발권을 진행해야 하며, 여권 유효기간이 6개월 이상 남아있는지 확인이 필요합니다. 수하물 규정 등 상세 정보는 첨부 문서를 통해 확인할 수 있습니다.", "일정 확인 및 여권 유효기간 재확인", "Medium", "결제"
    elif "점검" in title_lower or "서버" in title_lower or "장애" in title_lower:
        return "사내 시스템의 정기 점검 및 업데이트 예정 안내입니다. 예정된 시간 동안 서비스 접속과 관리 콘솔 이용이 일시적으로 제한될 수 있으므로 이를 고려하여 작업을 계획할 필요가 있습니다. 점검은 시스템 안정성 향상과 보안 패치 적용을 위해 진행됩니다.", "공지 시간 확인 및 작업 일정 조정", "High", "보안"
    elif "미팅" in title_lower or "회의" in title_lower or "일정" in title_lower:
        return "외부 협력사와의 신규 미팅 일정 조율 안내입니다. 협력사에서 제안한 여러 시간대 중 참석 가능한 일정을 회신해야 하며, 회의실 예약과 다과 준비는 발신자가 담당할 예정입니다. 빠른 회신이 일정 확정에 필요합니다.", "참석 가능 일정 회신 및 일정 확정", "High", "업무"
    elif "보안" in title_lower or "비밀번호" in title_lower or "권한" in title_lower:
        return "사내 PC 정기 보안 점검 수행 요청입니다. 백신 프로그램을 최신 버전으로 업데이트한 후 정밀 검사를 실행해야 하며, 검사 완료 후 스크린샷을 보안팀에 제출해야 합니다. 미제출 시 사내 망 접속이 차단될 수 있으므로 기한 준수가 중요합니다.", "백신 업데이트 및 정밀 검사 수행 후 결과 제출", "High", "보안"
    else:
        return f"'{title}'에 관한 이메일 안내입니다. 발신자의 의도와 요청사항을 파악하기 위해 메일 본문의 상세 내용을 확인하실 필요가 있습니다. 필요한 경우 적절한 조치나 회신을 진행하시기 바랍니다.", "메일 본문 확인 및 필요 조치", "Medium", "기타"
